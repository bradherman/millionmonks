class VotesController < ApplicationController
  ...
  def create
    ...
    if !current_user
      redirect_to login_path, :notice => "Please log in to vote."
    else
      respond_to do |format|
        if current_user.vote(@wisdom, params[:vote])
          format.rjs  { render :action => "create", :vote => @vote }
          format.html { redirect_to([@wisdom.user, @wisdom]) }
  ...
  def find_votes_for_my_scope
    if params[:quote_id]
      @votes = Vote.for_voteable(Wisdom.find(params[:wisdom_id])).descending
